nn=15; % кількість міст
asz=10; % розмір області asx x asz
ps=30; % чисельність популяції
ng=50; % Кількість поколінь
...
for gc=1:ng % цикл поколінь
   ...
   % схрещування:
    ii=indexes(ps,p); % гени з номерами міст ii будуть використані в схрещуванні
    ...
    for prc=1:(ps/2) % підрахунок пар
        ...
        % двоє дітей:
        g1ch=begining(g1,g2,cp);
        g2ch=begining(g2,g1,cp);
        Gch(i1,:)=g1ch;
        Gch(i2,:)=g2ch;
    end
    G=Gch; % нові діти
