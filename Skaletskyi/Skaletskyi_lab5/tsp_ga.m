...
ps=200; % чисельність популяції (бути кратнім 4)
...
for gc=1:ng % цикл поколінь
   ...
   %селекція 1111111111111
   pthd2=pthd;
   for i=1:ps/2
       [minF iminF]=min(pthd2);    
       Gs(i,:)=G(iminF,:);
       pthd2(iminF)=999999;
       pthd3(i)=pthd(iminF);
   end
   G=Gs;
   pthd=pthd3;
   % схрещування: 2222222222
   Gch=zeros(ps/2,nn); % діти
   for psc=1:(ps/4)
       i1=1+2*(psc-1);
       i2=2+2*(psc-1);
       g1=G(i1,:); %перший ген
   ...
   G(ps/2+1:ps,:)=Gch(:,:);
   size(G);
   % мутація золотого перетину: 3333333333
   for psc=1:ps
       if rand<pm
           n=ceil(nn*0.61803);
